...
node {
    ...
    stage('Switch the new blue server'){
        ...
        dir("${tf_path}"){
            new_green_id= sh returnStdout: true, script: "${terraform} state show aws_lb_target_group_attachment.blue_attach | grep target_id | awk '{print ${options}}' | tr -d '\n'"
            new_blue_id= sh returnStdout: true, script: "${terraform} state show aws_lb_target_group_attachment.green_attach | grep target_id | awk '{print ${options}}' | tr -d '\n'"
